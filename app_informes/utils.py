"""
Funciones de utilidad y helpers para la aplicación informes
"""
...
def obtener_datos_cookies(request):
    ...
    # 1) PRIMERO: Verificar datos de conexión (sin requerir token)
    datos_conexion = None
    
    # Intentar obtener desde connection_config (JSON)
    if config:
        try:
    ...
    if not datos_conexion:
        ...
        if empresa_ip and empresa_puerto:
            try:
                ...
                return None, None, None, "No se encontraron datos de conexión válidos. No se puede determinar a dónde realizar la consulta."
        else:
            print(f"❌ No se encontró connection_config ni cookies individuales válidas")
            return None, None, None, "No se encontraron datos de conexión. No se puede determinar a dónde realizar la consulta."

    # Verificar que los datos de conexión tengan IP y puerto válidos
    if not datos_conexion.get('ip') or not datos_conexion.get('puerto'):
        return None, None, None, "Los datos de conexión están incompletos. Falta IP o puerto."

    # 2) SEGUNDO: Verificar token
    if not token:
        return None, datos_conexion, None, "No se encontró token de autenticación. Por favor, inicie sesión nuevamente."

    # 3) TERCERO: Verificar usuario activo
    if not usuario:
        return token, datos_conexion, None, "No hay usuario activo. Por favor, inicie sesión nuevamente."

    # Todo está bien
    return token, datos_conexion, usuario, None
...
def renderizar_error(request, mensaje, empresa_nombre='', redirect_to=None, redirect_delay=5):
    ...
    return render(request, 'app_informes/informes.html', context)
